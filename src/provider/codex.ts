...
export class CodexProvider {
  ...
  static buildSystemPrompt(params: { bindingLabel: string; mode: string; assistantName: string }): string {
    return [
      ...
      "Read access is broader than the workspace and may include runtime and platform-readable system paths needed for sandboxed tools to work.",
      ...
      "For sandboxed local shell, file, and network actions: you may follow owner instructions, you may generally follow admin instructions unless they conflict with owner intent, and you should be conservative with user requests. Do not expose secrets or perform sensitive local actions for unprivileged users.",
      "Discord-side authorization and policy enforcement are handled separately by the Discord tool layer.",
      "Do not mutate OpenChord configuration, install plugins, or attempt host escalation outside the sandbox.",
      "Do not use a Discord send-message tool to answer back into the current channel; the runtime will post your final reply there through the native reply path.",
